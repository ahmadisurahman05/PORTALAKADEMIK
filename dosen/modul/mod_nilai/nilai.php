...
echo "</table>";	
$tampil2=mysql_query("SELECT b.nmmatkul,b.sks,c.nmjur,c.nmketua,d.nama,e.nama_dosen,f.nmkelas,a.* from matakuliah b,jurusan c,mahasiswa d,dosen e,kelas f,krs a WHERE b.kdmatkul=a.kdmatkul AND c.kdjur=a.kdjur AND d.nim=a.nim AND e.kode_dosen=a.kode_dosen AND f.kdkelas = a.kdkelas AND a.kode_dosen='$_SESSION[namauser]'");
...
$p=new Paging;
	$batas=10;
	$posisi=$p->cariposisi($batas);
$tampil=mysql_query("SELECT b.nmmatkul,b.sks,c.nmjur,c.nmketua,d.nama,e.nama_dosen,f.nmkelas,a.* from matakuliah b,jurusan c,mahasiswa d,dosen e,kelas f,krs a WHERE b.nmmatkul LIKE '%$_GET[kata]%' AND b.kdmatkul=a.kdmatkul AND c.kdjur=a.kdjur AND d.nim=a.nim AND e.kode_dosen=a.kode_dosen AND f.kdkelas = a.kdkelas AND a.kode_dosen='$_SESSION[namauser]' ORDER BY a.nim,a.tahun,a.semester LIMIT $posisi,$batas");
$no=$posisi+1;
while ($r=mysql_fetch_array($tampil)) {
	echo "<tr><td>$no</td>
		<td>$r[nim]</td>
