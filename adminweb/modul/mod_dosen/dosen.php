...
switch($act) {
	...
	<input type=button value='Tambah Dosen' onclick=location.href='?module=dosen&act=tambahdosen'>";
if (empty(@$_GET['kata'])){
echo "<table>
<tr>
	<th>No.</th>
	...
	$tampil = mysql_query("SELECT * FROM dosen ORDER BY kode_dosen limit $posisi,$batas");
	...
	while($r=mysql_fetch_array($tampil)){
		...
		echo "<tr><td>$no</td>
				...
				<a href=$aksi?module=dosen&act=hapus&id=$r[kode_dosen]><img src='images/hapus-icon.gif' alt='hapus' /></a></td>
			</tr>";
	$no++;
}
echo "</table>";
...
<tr>
	...
	$tampil = mysql_query("SELECT * FROM dosen WHERE nama_dosen LIKE '%$_GET[kata]%' ORDER BY kode_dosen limit $posisi,$batas");
...
echo "</table>";
	...
	$tampil2=mysql_query("select * from dosen WHERE nama_dosen LIKE '%$_GET[kata]%' ORDER BY kode_dosen");
	$jmldata=mysql_num_rows($tampil2);
...
<tr>
	<td>Kode Dosen</td>
	<td> : <input type=text name=kode_dosen size=18 max=18 value='$r[kode_dosen]'></td>
</tr>
<tr>
	<td>Nama Dosen</td>
