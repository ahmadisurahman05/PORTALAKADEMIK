...
switch($act) {
	...
	<form method=get action='$_SERVER[PHP_SELF]'>
		  ...
		  <input type=button value='Tambah Mahasiswa' onclick=\"window.location.href='?module=mahasiswa&act=tambahmahasiswa';\">";
if (empty($_GET['kata'])){
...
$tampil=mysql_query("SELECT * FROM mahasiswa,jurusan WHERE mahasiswa.kdjur=jurusan.kdjur ORDER BY nim limit $posisi,$batas");
$no=$posisi+1;
while($r=mysql_fetch_array($tampil)){
		...
		echo "<tr><td>$no</td>
				...
				<a href=$aksi?module=mahasiswa&act=hapus&id=$r[nim]><img src='images/hapus-icon.gif' alt='hapus' /></a></td>
			</tr>";
	$no++;
}
echo "</table>";
$tampil2=mysql_query("SELECT * FROM mahasiswa,jurusan WHERE mahasiswa.kdjur=jurusan.kdjur ORDER BY nim");
...
$tampil=mysql_query("SELECT * FROM mahasiswa,jurusan WHERE nama LIKE '%$_GET[kata]%' AND mahasiswa.kdjur=jurusan.kdjur ORDER BY nim limit $posisi,$batas");
$no=$posisi+1;
while($r=mysql_fetch_array($tampil)){
		$tglhr = tgl_indo($r['tglhr']);
		echo "<tr><td>$no</td>
...
echo "</table>";
$tampil2=mysql_query("SELECT * FROM mahasiswa,jurusan WHERE nama LIKE '%$_GET[kata]%' AND mahasiswa.kdjur=jurusan.kdjur ORDER BY nim");
	$jmldata=mysql_num_rows($tampil2);
...
<tr>
	<td>Alamat</td>
	<td> : <input type=textarea name=alamat maxlength=200></td>
</tr>
<tr>
	...
	combotgl(1989,$thn_skrg,'tahun',$thn_skrg);
echo "</td>
</tr>
<tr>
<td>Agama</td>
...
<tr>
	<td>NIM Mahasiswa</td>
	<td> : <input type=text name=nim size=18 maxlength=18 value='$r[nim]'></td>
</tr>
<tr>
	<td>Password</td>
	<td> : <input type=text name=password size=30 maxlength=30> *) </td>
...
<tr>
	...
	$thn_skrg=date("Y");
	combotgl2(1989,$thn_skrg,'tahun',$get_thn);
