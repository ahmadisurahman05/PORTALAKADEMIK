...
<tr>
<td colspan=2><input type=submit value='Tampilkan Jadwal'> <input type=button value='Tambah Jadwal' onclick=location.href='?module=jadwal&act=tambahjadwal'></td>
</tr>
</table>
</form>";
break;
...
while($r=mysql_fetch_array($tampiljdwl)) {
	echo "<tr><td>$no</td>
			...
			<td>$r[jam_selesai]</td>
			<td>$r[kode_dosen]</td>
			<td><a href=?module=jadwal&act=editjadwal&id=$r[id_jadwal]><img src='images/edit-icon.gif' alt='edit' /></a> &nbsp; 
				<a href=$aksi?module=jadwal&act=hapus&id=$r[id_jadwal]><img src='images/hapus-icon.gif' alt='hapus' /></a></td>
			</tr>";
		$no++;
}
echo "</table>";
